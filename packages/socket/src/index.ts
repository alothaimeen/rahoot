...
io.on("connection", (socket) => {
  ...
  socket.on("player:reconnect", ({ gameId }) => {
    ...
    socket.emit("game:reset", "اللعبة غير موجودة")
  })
  ...
  socket.on("manager:reconnect", ({ gameId }) => {
    ...
    socket.emit("game:reset", "انتهت صلاحية الجلسة")
  })
  ...
  socket.on("manager:auth", () => {
    try {
      ...
      socket.emit("manager:errorMessage", "فشل في قراءة إعدادات اللعبة")
    }
  })
  ...
  socket.on("game:create", (quizzId) => {
    ...
    if (!quizz) {
      socket.emit("game:errorMessage", "المسابقة غير موجودة")
  ...
  socket.on("player:join", (inviteCode) => {
    ...
    if (!game) {
      socket.emit("game:errorMessage", "اللعبة غير موجودة")
