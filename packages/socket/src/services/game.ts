...
class Game {
  ...
  join(socket: Socket, username: string) {
    ...
    if (isAlreadyConnected) {
      socket.emit("game:errorMessage", "هذا اللاعب متصل بالفعل")
  ...
  kickPlayer(socket: Socket, playerId: string) {
    ...
    this.io
      .to(player.id)
      .emit("game:reset", "تم طردك من اللعبة من قِبَل المدير")
  ...
  private reconnectManager(socket: Socket) {
    if (this.manager.connected) {
      socket.emit("game:reset", "المدير متصل بالفعل")
  ...
  private reconnectPlayer(socket: Socket) {
    ...
    if (player.connected) {
      socket.emit("game:reset", "هذا اللاعب متصل بالفعل")
  ...
  async start(socket: Socket) {
    ...
    if (this.players.length === 0) {
      socket.emit("game:errorMessage", "لا يوجد لاعبون متصلون بعد")
